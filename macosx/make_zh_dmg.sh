...
2. 第一次執行會出現「Apple 無法驗證…是否為惡意軟體」(沒有付費簽章的正常現象,二選一,只需做一次):
   * 作法 A(推薦,最快):打開「終端機」,貼上以下指令後按 Enter(路徑請改成你放的位置):
     xattr -dr com.apple.quarantine ~/Games/Ultima7_BlackGate_zhTW_v1.1_Portable
   * 作法 B:警告視窗按「完成」→ 系統設定 → 隱私權與安全性 → 往下捲到「安全性」
     → 『已封鎖「Exult」』按「仍要打開」→ 輸入密碼 → 再按「打開」。
   (較舊的 macOS 也可對 Exult.app 按住 Control 點一下 → 「打開」;新版系統已移除此捷徑)
