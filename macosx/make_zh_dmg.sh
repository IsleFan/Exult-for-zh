...
2. 第一次執行會出現「Apple 無法驗證…是否為惡意軟體」(沒有付費簽章的正常現象,只需做一次):
   打開「終端機 (Terminal)」,輸入以下文字(結尾有一個空格,先不要按 Enter):

       xattr -dr com.apple.quarantine

   然後把整個「Ultima7_BlackGate_zhTW_v1.1_Portable」資料夾**拖進終端機視窗**
   (路徑會自動填上),再按 Enter。完成後即可雙擊 Exult.app。
